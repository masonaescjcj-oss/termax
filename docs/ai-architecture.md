...
## ۳. جا شدن همه‌چیز روی یک سرور ۲۰–۳۰ دلاری

بودجه‌ی ۲۰ تا ۳۰ دلار امروز یک ماشین ۴ هسته‌ای با ۸ گیگابایت رم و ۱۶۰ گیگابایت
NVMe می‌دهد (مثلاً Hetzner CPX31، حدود ۱۸ تا ۲۵ دلار، با ۲۰ ترابایت ترافیک در
مناطق اروپا). سؤال درست «جا می‌شود؟» نیست، «اول چه چیزی می‌شکند؟» است.

### بودجه‌ی رم — با ۲۰۰۰ کاربر، ۳۰۰ نفر هم‌زمان، ۱۰۰۰ ربات

| مؤلفه | رم |
|---|---|
| سیستم‌عامل (دبیان حداقلی) | ~۲۵۰ MB |
| Caddy (پروکسی + بیلد وب استاتیک) | ~۲۵ MB |
| Node: runtime پایه + Express + Socket.IO | ~۹۰ MB |
| وضعیت بازار — ۳۰۰ سری × ۵۰۰ کندل در `Float64Array` | ~۷ MB |
| انباشتگرهای اندیکاتور (۳۰۰ × ۲۵ پیکربندی) | ~۱ MB |
| مشخصات و وضعیت ۱۰۰۰ ربات | ~۲ MB |
| ۳۰۰ سوکت هم‌زمان (~۴۰ KB هر کدام) | ~۱۲ MB |
| استخر QuickJS (۸ نمونه) | ~۱۲ MB |
| ورکرهای بک‌تست (۲ عدد، با پنجره‌ی کندل) | ~۱۶۰ MB |
| فضای آزاد هیپ V8 و سرریز GC | ~۲۵۰ MB |
| **جمع** | **~۸۱۰ MB** |
| Postgres محلی، اگر روی همین ماشین بیاید | +~۱.۳ GB |

دیسک: سیستم و کد و `node_modules` حدود ۳ گیگ، کندل‌های ۵۰ نماد برای ۳ سال روی
یک‌دقیقه حدود ۱.۵ تا ۴ گیگ بسته به فشرده‌سازی، آپلودها چند گیگ — جمعاً حدود ۱۵
گیگابایت از ۱۶۰.

**حافظه اصلاً نزدیک هم نیست.** حتی با دیتابیس روی همان ماشین، ۶ گیگابایت آزاد
می‌ماند.

### اول چه چیزی می‌شکند

۱. **عمق صف بک‌تست.** یک بک‌تست ۳ ساله روی ۱۵ دقیقه حدود ۲ ثانیه پردازنده
   می‌خورد؛ با ۲ ورکر یعنی حدود یک بک‌تست در ثانیه. اولین چیزی که کاربر حس
   می‌کند. حل: کش نتیجه و سقف بک‌تست هم‌زمان به‌ازای هر کاربر.
۲. **پردازنده‌ی پخش قیمت.** با batch، حدود ۲۰۰۰ تا ۳۰۰۰ اتصال هم‌زمان روی ۴ هسته
   راحت است. بدون batch، همان ۳۰۰ نفر هم پردازنده را می‌خورند.
۳. **محدودیت‌های Supabase.** پلن رایگان ۵۰۰ مگابایت دیتابیس و ۵ گیگابایت egress
   در ماه دارد و بعد از یک هفته بی‌فعالیتی pause می‌شود.

آنچه در این فهرست **نیست** هم مهم است: **تعداد ربات سقف نیست.** ده هزار ربات
اعلانی هم حدود ۵۰ مگابایت است.

### پنج قاعده‌ای که این را ممکن می‌کنند

**۱. بک‌تست هرگز روی thread اصلی نه.** Node تک‌threadی است. یک بک‌تست ۲ ثانیه‌ای
همگام یعنی ۲ ثانیه که هیچ تیک قیمتی پردازش نمی‌شود، هیچ حد ضرری شلیک نمی‌کند و
هیچ درخواستی سرویس نمی‌گیرد. در یک اپ معاملاتی قابل قبول نیست. بک‌تست باید در
`worker_threads` با استخر کوچک و یک صف اجرا شود، با سقف روی تعداد کندل هر اجرا و
یک بک‌تست هم‌زمان به‌ازای هر کاربر. این بهینه‌سازی نیست؛ شرط درست کار کردن است.

**۲. پخش قیمت را batch کنید.** کد فعلی در `sockets/marketSocket.ts` هر ثانیه برای
هر نماد به هر سوکت یک پیام می‌فرستد. ۳۰۰ کاربر × ۲۰ نماد = ۶۰۰۰ پیام در ثانیه.
یک پیام در هر بازه برای هر کلاینت شامل فقط نمادهای تغییرکرده‌ی خودش، همان را به
۳۰۰ پیام می‌رساند — بیست برابر کمتر. رابط کاربری سریع‌تر از ~۴ بار در ثانیه
نمی‌تواند از داده استفاده کند، پس بازه‌ی ۲۵۰ms تا ۱s چیزی از دست نمی‌دهد.

**۳. آرایه‌های تایپ‌دار، نه آرایه‌ی آبجکت.** بافر حلقوی کندل باید `Float64Array`
باشد. تفاوت فقط ۴–۵ برابر حافظه نیست — **فشار GC صفر می‌شود**. توقف garbage
collector داخل حلقه‌ی تیک دقیقاً همان چیزی است که روی ماشین کوچک نباید داشت.

**۴. کندل روی دیسک، هرگز در دیتابیس.** یک ردیف کندل در Postgres با سرریز و
ایندکس ۶۰ تا ۱۰۰ بایت است؛ همان کندل در فایل باینری ۴۸ بایت و بدون سرریز. سه سال
داده‌ی یک‌دقیقه‌ای برای ۵۰ نماد در دیتابیس ده‌ها گیگابایت و کند می‌شود، روی دیسک
چند گیگابایت و یک خواندن ترتیبی. یک فایل به‌ازای هر نماد در هر ماه، و تایم‌فریم
بالاتر در لحظه‌ی خواندن از یک‌دقیقه ساخته شود.

**۵. وضعیت زنده در حافظه، دیتابیس برای دوام.** گران‌ترین مورد در کد فعلی و
آسان‌ترین برد. `getAccountState` در حلقه‌ی تیک پوزیشن‌ها و حساب را از دیتابیس
می‌خواند — به‌ازای هر پوزیشن دو رفت‌وبرگشت به Supabase. با ۳۰۰ کاربر هم egress
پلن رایگان را می‌خورد و هم تأخیر وارد مسیری می‌کند که حد ضرر در آن اجرا می‌شود.
موتور از قبل `positionMap` را در حافظه دارد؛ حساب‌ها هم باید همان‌جا باشند.

### دیتابیس: انتخابی که باید بکنید

اگر ۲۰ تا ۳۰ دلار *کل* بودجه است، Supabase Pro با ماهی ۲۵ دلار به‌تنهایی تمامش
می‌کند.

| گزینه | جمع ماهانه | معامله |
|---|---|---|
| Supabase رایگان | ۱۸–۲۵ دلار | کار می‌کند اگر دیتابیس زیر ۵۰۰ MB بماند (با قاعده‌ی ۴ می‌ماند). سقف واقعی ۵ گیگابایت egress ماهانه است، پس قاعده‌ی ۵ ضروری می‌شود. |
| Supabase Pro | ۴۳–۵۰ دلار | خارج از بودجه، اما بی‌دردسر: ۸ GB دیتابیس، پشتیبان‌گیری، بدون pause. |
| Postgres روی همان سرور | ۱۸–۲۵ دلار | در بودجه می‌ماند و سقف egress را حذف می‌کند. در عوض پشتیبان‌گیری و ارتقا کار خودتان می‌شود و `models/*.ts` باید از کلاینت Supabase به SQL مستقیم مهاجرت کند — کار متوسطی است چون آن shellها نازک‌اند. |

### و بزرگ‌ترین هزینه، سرور نیست

رو و ساده: **توکن هوش مصنوعی می‌تواند یک مرتبه‌ی بزرگی از سرور گران‌تر شود.** یک
نوبت گفت‌وگو با استفاده از ابزار، حتی با کش، چیزی حدود ۱ تا ۲ سنت است. سیصد
کاربر فعال با روزی ده نوبت یعنی ماهی نود هزار نوبت — صدها دلار، نه بیست‌وپنج
دلار.

پس محدودیت واقعی بودجه‌ی شما سرور نیست؛ AI است. اهرم‌ها: سهمیه‌ی مشخص در پلن
رایگان (مثلاً ۵ پیام در روز و بعد پی‌وال)، مدل کوچک برای اکثریت نوبت‌ها که فقط
جست‌وجو و مسیریابی نیت‌اند، کش کردن پیشوند ثابت، پیش‌محاسبه‌ی آمار، و سقف روی
تعداد دور فراخوانی ابزار در هر نوبت.

---

## ۴. کد سفارشی: لایه‌ی پیشرفته
